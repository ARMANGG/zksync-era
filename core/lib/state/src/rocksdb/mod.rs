...
pub struct SynchonizingRocksdb {
    rocksdb_sync_handle: JoinHandle<anyhow::Result<bool>>,
}
        ...
        match self {
            SecondaryStorage::NotInitialized => {
                *self = Self::initialize(
                ...
                .await?;
                Ok(Some(Self::access_storage_pg(rt_handle, pool).await?))
            }
            SecondaryStorage::Synchronizing(SynchonizingRocksdb {
                rocksdb_sync_handle,
            }) => {
                if rocksdb_sync_handle.is_finished() {
                    let rocksdb_sync_handle = std::mem::replace(
                        rocksdb_sync_handle,
                        tokio::task::spawn_blocking(|| Ok(true)),
                    );
                    if rocksdb_sync_handle.await?? {
                        *self = SecondaryStorage::Synchronized(SyncrhonizedRocksdb {});
                        Ok(Some(Self::access_storage_pg(rt_handle, pool).await?))
                    } else {
                        *self = SecondaryStorage::NotInitialized;
                        Ok(None)
                    }
                } else {
                    Ok(Some(Self::access_storage_pg(rt_handle, pool).await?))
            ...
            SecondaryStorage::Synchronized(SyncrhonizedRocksdb {}) => {
                ...
                let current_l1_batch_number = rocksdb
                    ...
                    .await
                    .unwrap();
                if current_l1_batch_number + 1000 < latest_l1_batch_number {
                    *self = Self::initialize(
                        pool.clone(),
                        state_keeper_db_path,
                        enum_index_migration_chunk_size,
